...
<div class="col-12">
    <div class="card">
        ...
        <h4 class="card-title">{{ trans('rent.add_rentant') }}</h4>                                
        </div>
        <div class="card-content">
            <div class="card-body">
                <div class="row">    
                    ...
                    <div class="col-12">
                        <form action="{{ route('rentants.store') }}" method="post">
                            @csrf
                            <div class="form-group">
                            <label for="username1">{{trans('rent.rentant_name')}}</label>                                                  
                                <input type="text" class="form-control @error('name') is-invalid @enderror" id="username1" name="name" placeholder="{{trans('rent.rentant_name')}}">
                                @error('name')
                            ...
                            <label for="email1">{{trans('rent.age')}}</label>                                                   
                                ...
                                name="age" placeholder="{{trans('rent.age')}}">
                                @error('age')
                                <span class="text-danger">{{$message}}</span>
                                    @enderror  
                            </div>
                            ...
                            <label for="inputPassword31">{{trans('rent.activity')}}</label>                                                    
                                ...
                                name="activity" 
                                placeholder="{{trans('rent.activity')}}">        
                                @error('activity')
                            <span class="text-danger">{{$message}}</span>
                                @enderror        
                            </div> 
                            <div class="form-group">                                                  
                            <button type="submit" class="btn btn-primary">{{ trans('rent.add_rentant') }}</button></button>
                            </div>
                        </form>
                    </div>
                </div>
