...
<div class="col-12">
    <div class="card">
        ...
        <h4 class="card-title">{{ trans('rent.edit_lease') }}</h4>                                
        </div>
        <div class="card-content">
            <div class="card-body">
                <div class="row">    
                    ...
                    <div class="col-12">
                        <form action="{{ route('leases.update' , $item->id) }}" method="post">
                            @csrf
                            @method('put')
                            ...
                            <option value="{{$apart->id}}" {{$item->apartment_id == $apart->id ? 'selected' : ''}}>{{$apart->name}}</option>  
                                @empty
                            ...
                            <?php $rentants = \App\Models\Rentant::all(['id' , 'name']); ?>      
                            <select name="rentant_id" id="apartment" class="form-control @error('rentant_id') is-invalid @enderror" id="username1">                                      
                                ...
                                @forelse ($rentants as $person)
                            <option value="{{$person->id}}" {{$item->rentant_id == $person->id ? 'selected' : ''}}>{{$person->name}}</option>
                                @empty
                            ...
                            name="duration" placeholder="{{trans('rent.duration')}}" value="{{$item->duration}}">
                                @error('duration')
                                <span class="text-danger">{{$message}}</span>
                                    @enderror  
                            </div>
                            ...
                            <label for="inputPassword31">{{trans('rent.agreed_money')}}</label>                                                    
                                ...
                                name="agreed_money" 
                                placeholder="{{trans('rent.agreed_money')}}" value="{{$item->agreed_money}}">  
                                @error('agreed_money')
                                <span class="text-danger">{{$message}}</span>
                                    @enderror              
                            </div> 
                            ...
                            <label for="inputPassword31">{{trans('rent.start_from')}}</label>                                                    
                                ...
                                name="start_from" 
                                placeholder="{{trans('rent.start_from')}}" value="{{$item->start_from}}">        
                                @error('start_from')
                            <span class="text-danger">{{$message}}</span>
                                @enderror        
                            </div> 
                            <div class="form-group">                                                  
                            <button type="submit" class="btn btn-primary">{{ trans('rent.edit_lease') }}</button></button>
                            </div>
                        </form>
                    </div>
                </div>
