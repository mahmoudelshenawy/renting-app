...
<div class="col-12">
    <div class="card">
        ...
        <h4 class="card-title">{{ trans('rent.edit_apartment') }}</h4>                                
        </div>
        <div class="card-content">
            <div class="card-body">
                <div class="row">                                        
                    <div class="col-12">
                        <form action="{{ route('apartments.update' , $item->id) }}" method="post">
                            @csrf
                            @method('put')
                            <div class="form-group">
                            <label for="username1">{{trans('rent.apartment_name')}}</label>                                                  
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="username1" name="name" placeholder="{{trans('rent.apartment_name')}}" value="{{$item->name}}">
                                @error('name')
                            ...
                            <label for="email1">{{trans('rent.code')}}</label>                                                   
                                ...
                                name="code" placeholder="{{trans('rent.code')}}" value="{{$item->code}}">
                                @error('code')
                                <span class="text-danger">{{$message}}</span>
                                    @enderror  
                            </div>
                            ...
                            <label for="inputPassword31">{{trans('rent.apartment_area')}}</label>                                                    
                                ...
                                name="apartment_area" 
                                placeholder="{{trans('rent.apartment_area')}}" value="{{$item->apartment_area}}">  
                            ...
                            <label for="inputPassword31">{{trans('rent.floor')}}</label>                                                    
                                ...
                                name="floor_id" 
                                placeholder="{{trans('rent.floor')}}" value="{{$item->floor_id}}">        
                                @error('floor_id')
                            <span class="text-danger">{{$message}}</span>
                                @enderror        
                            </div> 
                            <div class="form-group">                                                  
                            <button type="submit" class="btn btn-primary">{{ trans('rent.edit_apartment') }}</button></button>
                            </div>
                        </form>
                    </div>
                </div>
